		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("ManualNotifId") === oObject.ManualNotifId;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].ManualNotifId;
									} else {
										sCurrentId = aItemsNotInTheList[0].ManualNotifId;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("ManualNotifId"),
						title: oBindingContext.getProperty("Instid")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "NotificationInfoSet (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'NotificationInfoSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Instid");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all NotificationInfoSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "NotificationInfoSet (" + iExpectedLength + ")"
								}),
								success: function () {
